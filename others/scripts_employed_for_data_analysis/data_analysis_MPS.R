...
round(rowMeans(METH_cov[POS_450K, !(1:ncol(METH_cov) %in% remove)]), 1)
# cg01940273_170_170   cg03636183_41_41   cg05575921_59_59 cg05951221_210_210   cg06126421_66_66   cg09935388_50_50   cg12803068_33_33 
# 9678.1             8559.4            16252.6            22637.1             5270.3             1656.8             2546.8 
# cg12876356_205_205   cg13039251_33_33 cg15693572_120_120   cg21566642_71_71   cg22132788_97_97   cg23576855_52_52 
# 16923.9             6708.4             6263.6              338.2            17234.2            13629.6 
round(rowSds(METH_cov[POS_450K, !(1:ncol(METH_cov) %in% remove)]), 1)
# [1]  6572.7  6474.5 12561.2 16254.5  3869.9  1099.2  1853.0 12674.2  4881.1  4343.6   221.6 12273.9  9793.3

# colnames(METH_cov)[remove] # S45
# grep('S58', colnames(METH_cov)) # integer(0)
# grep('S73', colnames(METH_cov)) # integer(0)
...
round(rowMeans(METH_cov[POS_450K, !(1:ncol(METH_cov) %in% remove)]), 1)
# cg01940273_170_170   cg03636183_41_41   cg05575921_59_59 cg05951221_210_210   cg06126421_66_66   cg09935388_50_50   cg12803068_33_33 
# 8770.4             8755.8            28075.8            21719.9             7211.9              845.1             1897.0 
# cg12876356_205_205   cg13039251_33_33 cg15693572_120_120   cg21566642_71_71   cg22132788_97_97   cg23576855_52_52 
# 21131.6             7988.6             7986.6              517.6            17326.7            14988.4 
round(rowSds(METH_cov[POS_450K, !(1:ncol(METH_cov) %in% remove)]), 1)
# [1]  5365.4  4944.4 14559.1 12511.8  4224.4   482.0  1131.7 13101.5  7306.9  5354.6   375.2 11598.5  8447.4

# colnames(METH_cov)[remove] # S51, S54, S78
# grep('S19', colnames(METH_cov)) # integer(0)
# grep('S62', colnames(METH_cov)) # integer(0)
# grep('S64', colnames(METH_cov)) # integer(0)
...
round(rowMeans(METH_cov[POS_450K, !(1:ncol(METH_cov) %in% remove)]), 1)
# cg01940273_170_170   cg03636183_41_41   cg05575921_59_59 cg05951221_210_210   cg06126421_66_66   cg09935388_50_50   cg12803068_33_33 
# 8232.5            18310.0            16106.0            20911.1            16574.0             2541.3             3903.1 
# cg12876356_205_205   cg13039251_33_33 cg15693572_120_120   cg21566642_71_71   cg22132788_97_97   cg23576855_52_52 
# 12431.9             5457.4            11972.1             1364.9            30635.1            21399.7 
round(rowSds(METH_cov[POS_450K, !(1:ncol(METH_cov) %in% remove)]), 1)
# [1] 2000.9 4270.4 3918.7 5276.0 3357.1  785.5 1113.4 3149.5 1607.6 2598.3  481.1 7234.3 4291.9


# colnames(METH_cov)[remove] # S1, 2
...
# p = ggplot(data = cov.m, mapping = aes(x = CpG, y = log10(count+1), fill = CpG, linetype = run)) + geom_violin() +
#   labs(title = "All runs", x = "CpG", y = "log10(cytosine_count + 1)") + ylim(0, 5) + theme_minimal() +
#   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1), 
#         plot.title = element_text(hjust = 0.5)) + 
...
colnames(METH) %in% phenotype$ERF_ID
phenotype$ERF_ID[!startsWith(phenotype$ERF_ID, 'STD')]  %in% colnames(METH)
table((phenotype[!startsWith(phenotype$ERF_ID, 'STD'),])$sequencing_run, (phenotype[!startsWith(phenotype$ERF_ID, 'STD'),])$smoking_3cat)
jkl = phenotype[phenotype$ERF_ID[!startsWith(phenotype$ERF_ID, 'STD')]  %in% colnames(METH), ]
View(jkl)
...
cor(as.numeric(STD1[, colnames(STD2)]), as.numeric(STD2)) # 
...
bias = colSums(sapply(1:length(LEV), function(x) (STD$value[STD$Var2 == LEV[x]] - STD$ratio[STD$Var2 == LEV[x]])))
names(bias) = LEV
sort(bias)
# cg13039251  cg15693572  cg05951221  cg06126421  cg23576855  cg05575921  cg21566642  cg01940273  cg09935388  cg22132788  cg03636183  cg12803068 
# -3.78535828 -3.14326619 -2.55205956 -2.46862470 -1.98540014 -1.61735704 -1.42515952 -1.08933034  0.03525236  0.84735783  1.91241187  2.52394562 
# cg12876356 
# 4.79566882 

CpG_count = c(`cg05575921`= 9, `cg06126421` = 3, `cg22132788` = 23, `cg12803068` = 4, 
  `cg13039251` = 2, `cg01940273` = 10, `cg03636183` = 7, `cg23576855` = 7, 
  `cg12876356` = 21, `cg15693572` = 1, `cg05951221` = 25, `cg09935388` = 10, `cg21566642` = 23)
CpG_count = CpG_count[names(bias)]
plot(CpG_count, bias)
cor.test(CpG_count, bias)
# data:  CpG_count and bias
# t = 1.0403, df = 11, p-value = 0.3205
...
amplicon_length = c(`cg05575921`= 228, `cg06126421` = 194, `cg22132788` = 232, `cg12803068` = 137, 
                    `cg13039251` = 134, `cg01940273` = 188, `cg03636183` = 199, `cg23576855` = 186, 
                    `cg12876356` = 239, `cg15693572` = 160, `cg05951221` = 227, `cg09935388` = 125, `cg21566642` = 265)
amplicon_length = amplicon_length[names(bias)]
plot(amplicon_length, bias)
cor.test(amplicon_length, bias)
# data:  amplicon_length and bias
# t = 0.56001, df = 11, p-value = 0.5867
# alternative hypothesis: true correlation is not equal to 0
# 95 percent confidence interval:
#   -0.4233273  0.6571901
# sample estimates:
...
RES = aggregate(count~ CpG, cov.m, mean)
CpG_cov = RES$count; names(CpG_cov) = RES$CpG; CpG_cov = CpG_cov[names(bias)]
plot(CpG_cov, bias)
cor.test(CpG_cov, bias)
# data:  CpG_cov and bias
# t = 0.15577, df = 11, p-value = 0.879
# alternative hypothesis: true correlation is not equal to 0
# 95 percent confidence interval:
#   -0.5174463  0.5828340
# sample estimates:
...
X = X[!endsWith(rownames(X), '.1'),] # Remove technical replicates
X = X[X$smoke != '',]
...
cor(X[,c(colnames(X)[1:13], 'age')], method = 'pearson')[,'age']
#  cg01940273  cg03636183  cg05575921  cg05951221  cg06126421  cg09935388  cg12803068  cg12876356  cg13039251  cg15693572  cg21566642  cg22132788 
# -0.27514075 -0.23041215 -0.19710854 -0.24029417 -0.37715291 -0.18519944 -0.01722524 -0.15545815 -0.39823572 -0.31751496 -0.23388405 -0.02450649 
# cg23576855         age 
# -0.18791713  1.00000000
...
pval = sapply(1:(ncol(X)-2), function(x) summary(lm(X$age ~ X[,x] + factor(X$smoke)))$coefficients[2,'Pr(>|t|)'])
pval = p.adjust(pval, 'bonferroni')
names(pval) = colnames(X)[1:(ncol(X)-2)]
...
cor(as.numeric(METH_corrected[,indices[1,]]), as.numeric(METH_corrected[,indices[2,]])) # 0.8877868
outliers = which(abs(as.numeric(METH_corrected[,indices[1,]]) - as.numeric(METH_corrected[,indices[2,]])) > 0.3)
cor(as.numeric(METH_corrected[,indices[1,]])[-outliers], as.numeric(METH_corrected[,indices[2,]])[-outliers]) # 0.9799081
...
#  cg01940273  cg03636183  cg05575921  cg05951221  cg06126421  cg09935388  cg12803068  cg12876356  cg13039251  cg15693572  cg21566642  cg22132788 
# 1.000000000 0.867450832 0.968031210 1.000000000 0.099055154 0.001898682 1.000000000 1.000000000 1.000000000 1.000000000 0.138762927 0.371967421 
# cg23576855 
# 1.000000000 
...
dim(X[!is.na(X$n_cigarettes) & X$smoke != '', ]) # 229
...
p.adjust(pvals, 'bonferroni') < 0.001
p.adjust(pvals, 'bonferroni') < 0.05
...
length(X[!is.na(X$n_cigarettes),1]) # 237

length(X[!is.na(X$time_since_cessation),1]) # 72



## corr
cor = sapply(1:nrow(METH), function(x) cor(X[!is.na(X$n_cigarettes) & !is.na(df$smoke),x], X$n_cigarettes[!is.na(X$n_cigarettes) & !is.na(df$smoke)], use = 'complete.obs'))
names(cor) = colnames(X)[1:13]


cor = sapply(1:nrow(METH), function(x) cor(X[!is.na(X$time_since_cessation) & !is.na(df$smoke),x], X$time_since_cessation[!is.na(X$time_since_cessation) & !is.na(df$smoke)], use = 'complete.obs'))
names(cor) = colnames(X)[1:13]
...
table(phenotype[phenotype$ERF_ID %in% rep,]$smoking_3cat)/2
# current_smoker    past_smoker 
# 4                 2 
...
jkl = phenotype[match(colnames(X), phenotype$ERF_ID),]
dim(jkl) # 232
71+71+90

mean(jkl$age[jkl$smoking_3cat == 'non_smoker'])
sd(jkl$age[jkl$smoking_3cat == 'non_smoker'])
mean(jkl$age)
sd(jkl$age)

table(jkl$ERF_gender[jkl$smoking_3cat == 'current_smoker'])
44/(44+46)
...
RES = lapply(1:length(LEV), function(x) summary(lm(value ~ Smoking + platform, data = na.omit(df_all[df_all$variable == LEV[x],])))$coefficients)
RES = sapply(RES, function(x) x['platformseq', 'Pr(>|t|)'])
names(RES) = LEV
RES
# cg05575921    cg13039251    cg03636183    cg12803068    cg22132788    cg06126421    cg21566642    cg23576855    cg15693572    cg05951221 
# 2.720767e-23 6.493041e-166 5.248344e-193  5.424560e-01  5.122818e-89  2.436069e-01  6.644691e-72  5.384228e-19  4.755348e-27 3.775473e-126 
# cg01940273    cg12876356    cg09935388 
# 7.199805e-15 5.983972e-102  7.242481e-03 
...
RES
# cg05575921    cg13039251    cg03636183    cg12803068    cg22132788    cg06126421    cg21566642    cg23576855    cg15693572    cg05951221 
# 3.842496e-26 2.504095e-141  1.133161e-06 1.471822e-158 4.030309e-224 3.065154e-178  0.000000e+00 5.318418e-104 1.554342e-144  2.876445e-17 
# cg01940273    cg12876356    cg09935388 
# 2.525791e-195  0.000000e+00  3.625839e-01
RES < 0.05/13
...
model$resample
#           F1   Resample
# 1  0.9489576 Fold1.Rep1
# 2  0.9438040 Fold2.Rep1
# 3  0.9514286 Fold3.Rep1
# 4  0.9551375 Fold4.Rep1
# 5  0.9486623 Fold5.Rep1
# 6  0.9505376 Fold1.Rep2
# 7  0.9518332 Fold2.Rep2
# 8  0.9480519 Fold3.Rep2
# 9  0.9495677 Fold4.Rep2
# 10 0.9472162 Fold5.Rep2
...
model$resample
#           F1   Resample
# 1  0.6914599 Fold2.Rep1
# 2  0.7140544 Fold1.Rep1
# 3  0.6843838 Fold3.Rep1
# 4  0.6886136 Fold1.Rep2
# 5  0.7186631 Fold5.Rep1
# 6  0.6884197 Fold4.Rep1
# 7  0.7342760 Fold2.Rep2
# 8  0.6951731 Fold5.Rep2
# 9  0.6435052 Fold4.Rep2
# 10 0.7009948 Fold3.Rep2
...
tiff(filename = 'REAL_reps_3cat_conf.tiff', res = 300, width = 4, height = 4.1, units = 'in')
gplots::balloonplot(unname(table(factor(ghj, levels = c('non_smoker', 'past_smoker', 'current_smoker')), cat)), main = '')
dev.off()
